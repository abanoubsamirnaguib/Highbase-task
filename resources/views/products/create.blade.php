        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.products.store') }}" method="POST" id="productForm">
                    ...
                    {{-- Show old attribute errors --}}
                    @if($errors->hasAny(array_map(fn($k) => "attributes.$k", array_keys(old('attributes', [])))))
...
<script>
    ...
    function buildAttributeField(attr) {
        const oldVal = oldAttributes[attr.id] ?? '';
    ...
    // Auto-load if old category was selected (e.g., after failed validation)
